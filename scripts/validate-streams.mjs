...
async function probeWithFfmpeg(url) {
  const args = [...transportArgs(url), "-i", url, "-frames:v", "1", "-f", "null", "-"];
...
async function main() {
  ...
  if (!existsSync(sourcesPath)) {
    log(
      `${C.red}ERRO:${C.reset} arquivo não encontrado. Crie server/rtsp.sources.json (veja rtsp.sources.example.json).`,
    );
    process.exit(2);
  }
  ...
  log(
    `${C.dim}Ferramenta: ${tool} · timeout: ${PROBE_TIMEOUT_MS}ms · fontes: ${sources.length}${C.reset}\n`,
  );
  ...
  for (const src of sources) {
    ...
    const tag =
      src.verified === true
        ? `${C.cyan}[verificado]${C.reset} `
        : src.verified === false
          ? `${C.yellow}[não verificado]${C.reset} `
          : "";
    ...
    if (r.ok) {
      ...
      log(
        `  ${C.green}OK${C.reset}  ${ms}ms  resolução ${reso}${codec}${r.note ? ` ${C.dim}(${r.note})${C.reset}` : ""}\n`,
      );
    } else {
      failCount++;
    ...
    results.push({
      ...
      ok: r.ok,
      ms,
      reso: r.width ? `${r.width}x${r.height}` : "",
      note: r.note || "",
    });
  }

  log(
    `${C.bold}Resumo:${C.reset} ${C.green}${okCount} OK${C.reset} · ${C.red}${failCount} FALHA${C.reset} (de ${sources.length})`,
  );
