...
export type PhoneDetection = { x: number; y: number; width: number; height: number; score: number } | null;
export type HandDetection = { landmarks: Landmark[]; handedness: Handedness; signal: ManualSignal }[];
...
const HAND_INDEX_TIP = 8, HAND_INDEX_PIP = 6, HAND_MIDDLE_TIP = 12, HAND_MIDDLE_PIP = 10;
const HAND_RING_TIP = 16, HAND_RING_PIP = 14, HAND_PINKY_TIP = 20, HAND_PINKY_PIP = 18;
const HAND_THUMB_TIP = 4, HAND_THUMB_IP = 3, HAND_THUMB_MCP = 2;

function distance(a: Landmark, b: Landmark): number { return Math.hypot(a.x - b.x, a.y - b.y); }
...
export function calcEar(landmarks: Landmark[]): number | null {
  const l = eyeAspectRatio(landmarks, LEFT_EYE), r = eyeAspectRatio(landmarks, RIGHT_EYE);
  if (l == null || r == null) return null;
  return (l + r) / 2;
}

export function calcMar(landmarks: Landmark[]): number | null {
  const wA = landmarks[MOUTH_W[0]], wB = landmarks[MOUTH_W[1]], oA = landmarks[MOUTH_O[0]], oB = landmarks[MOUTH_O[1]];
...
function fingerExtended(l: Landmark[], tip: number, pip: number): boolean {
  const t = l[tip], p = l[pip];
  return !!t && !!p && t.y < p.y;
}
function thumbExtended(l: Landmark[], hand: Handedness): boolean {
  const tip = l[HAND_THUMB_TIP], ip = l[HAND_THUMB_IP], mcp = l[HAND_THUMB_MCP];
...
export function normalizedLandmarksRect(landmarks: Landmark[], width: number, height: number): Rect | null {
...
export function earZones(faceRect: Rect): Rect[] {
  const w = faceRect.width * 0.42, h = faceRect.height * 0.62, y = faceRect.y + faceRect.height * 0.12;
  return [
