...
export function drawFadigaScene(
  ...
  const frameColor =
    data.riskState === "ALERTA_DUPLO"
      ? "rgba(248,113,113,0.98)"
      : data.riskState === "OK"
        ? "rgba(34,197,94,0.95)"
        : "rgba(250,204,21,0.98)";
  const boxW = Math.round(cr.width * 0.42),
    boxH = Math.round(cr.height * 0.62);
  ctx.strokeStyle = frameColor;
  ctx.lineWidth = 2;
  ctx.strokeRect(
    Math.round(cr.x + (cr.width - boxW) / 2),
    Math.round(cr.y + (cr.height - boxH) / 2),
    boxW,
    boxH,
  );
  ...
  if (data.landmarks && data.landmarks.length) {
    ...
    const xs = pts.map((p) => p.x),
      ys = pts.map((p) => p.y);
    const minX = Math.max(cr.x, Math.min(...xs)),
      minY = Math.max(cr.y, Math.min(...ys));
    const maxX = Math.min(cr.x + cr.width, Math.max(...xs)),
      maxY = Math.min(cr.y + cr.height, Math.max(...ys));
  ...
  if (data.phone) {
    ...
    ctx.fillRect(r.x, Math.max(cr.y, r.y - 20), 120, 18);
    ctx.fillStyle = "#111827";
    ctx.font = "700 12px ui-sans-serif, system-ui";
    ctx.fillText(
      `Celular ${(data.phone.score * 100).toFixed(0)}%`,
      r.x + 6,
      Math.max(cr.y + 13, r.y - 7),
    );
  }
  ...
  for (const hand of data.hands) {
    ...
    if (hand.signal !== "SEM_SINAL") {
      ctx.fillStyle = "rgba(167,139,250,0.92)";
      ctx.fillRect(minX, Math.max(cr.y, minY - 20), 140, 18);
      ctx.fillStyle = "#111827";
      ctx.font = "700 12px ui-sans-serif, system-ui";
