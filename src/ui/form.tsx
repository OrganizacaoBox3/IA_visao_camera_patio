...
export function FieldLabel({ htmlFor, children }: { htmlFor?: string; children: ReactNode }) {
  return <Label.Root className="ui-label" htmlFor={htmlFor}>{children}</Label.Root>;
}
...
export function Field({ label, hint, error, htmlFor, className, children }: {
  label?: ReactNode; hint?: ReactNode; error?: ReactNode; htmlFor?: string; className?: string; children: ReactNode;
}) {
  return (
    <div className={cx("ui-field", className)}>
      {label && <FieldLabel htmlFor={htmlFor}>{label}</FieldLabel>}
      {children}
      {error ? <span className="ui-err">{error}</span> : hint ? <span className="ui-hint">{hint}</span> : null}
    </div>
  );
}
