...
export function decoderKind(): DecoderKind { return kind; }
...
async function init(): Promise<void> {
  ...
  try {
    ...
    if (BD) {
      ...
      } catch { /* alguns navegadores não expõem getSupportedFormats */ }
  ...
  try {
    ...
    worker.onmessage = (e: MessageEvent<{ id: number; code: string | null; format: string }>) => {
      ...
      if (cb) { pending.delete(e.data.id); cb(e.data.code ? { code: e.data.code, format: e.data.format } : null); }
    };
    worker.onerror = () => { worker = null; kind = "indisponível"; };
    kind = "zxing";
    return;
  } catch { worker = null; }
...
export async function decodeFromCanvas(canvas: HTMLCanvasElement): Promise<DecodeResult | null> {
  ...
  if (nativeDetector) {
    ...
    } catch { /* frame ruim/ocupado — ignora */ }
    return null;
  }
