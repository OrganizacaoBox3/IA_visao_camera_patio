...
import { MultiFormatReader, DecodeHintType, BarcodeFormat, RGBLuminanceSource, BinaryBitmap, HybridBinarizer } from "@zxing/library";

const reader = new MultiFormatReader();
const hints = new Map<number, unknown>();
hints.set(DecodeHintType.POSSIBLE_FORMATS, [
  BarcodeFormat.EAN_13, BarcodeFormat.EAN_8, BarcodeFormat.CODE_128, BarcodeFormat.CODE_39,
  BarcodeFormat.ITF, BarcodeFormat.QR_CODE, BarcodeFormat.DATA_MATRIX, BarcodeFormat.UPC_A, BarcodeFormat.UPC_E,
]);
...
self.onmessage = (e: MessageEvent<Req>) => {
  ...
  for (let i = 0, j = 0; i < data.length; i += 4, j++) lum[j] = (data[i] * 0.299 + data[i + 1] * 0.587 + data[i + 2] * 0.114) | 0;
  try {
    ...
    (self as unknown as Worker).postMessage({ id, code: res.getText(), format: BarcodeFormat[res.getBarcodeFormat()] });
  } catch {
