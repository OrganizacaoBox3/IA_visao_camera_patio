...
type DetMsg = { type: "detect"; id: number; rgba: ArrayBuffer; w: number; h: number; maxBoxes: number; minScore: number };

let model: cocoSsd.ObjectDetection | null = null;
let loadP: Promise<void> | null = null;

function ensure(base: Base): Promise<void> {
  if (!loadP) loadP = cocoSsd.load({ base }).then((m) => { model = m; });
  return loadP;
}
...
self.onmessage = async (e: MessageEvent<InitMsg | DetMsg>) => {
  ...
  if (m.type === "init") {
    try { await ensure(m.base); post({ type: "ready" }); }
    catch (err) { post({ type: "error", error: String(err) }); }
    return;
  }

  if (m.type === "detect") {
    if (!model) { post({ type: "result", id: m.id, dets: [] }); return; }
    try {
      ...
      const dets = res.map((d) => ({
        ...
        bbox: [d.bbox[0] / m.w, d.bbox[1] / m.h, d.bbox[2] / m.w, d.bbox[3] / m.h] as [number, number, number, number],
      }));
