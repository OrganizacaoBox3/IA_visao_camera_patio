...
import { detectObjects, ensureObjectDetector, objectBackend, type ObjDetection, type ObjBackend } from "../objects/detector";
...
export type ObjetosSetor = { id: string; label: string; x: number; y: number; w: number; h: number; contains?: (nx: number, ny: number) => boolean };
...
type Accum = { setor: string; classe: string; samples: number; countSum: number; peak: number; present: number };
...
export class ObjetosProcessor implements Disposable {
  ...
  process(setores: ObjetosSetor[], classes: string[], ctx: ObjetosCtx): ObjetosResult {
    ...
    let carregando = 0, cargaSetor = "";
    const pessoas = this.dets.filter((d) => d.key === "pessoa"), caixas = this.dets.filter((d) => d.key === "caixa");
    for (const p of pessoas) for (const cx of caixas) if (overlap(p.bbox, cx.bbox) > 0.02) { carregando++; if (!cargaSetor) cargaSetor = this.zoneOf(setores, p.bbox[0] + p.bbox[2] / 2, p.bbox[1] + p.bbox[3] / 2); break; }
    if (carregando > 0 && now - this.lastCargaEvt > 4000) {
      this.lastCargaEvt = now;
      events.push({ type: "carregamento", setor: cargaSetor || setores[0]?.label || "Cena", classe: "caixa", ts: Date.now() });
    }
    ...
    if (now - this.lastAccum > 1000) {
      ...
      for (const d of this.dets) { const z = this.zoneOf(setores, d.bbox[0] + d.bbox[2] / 2, d.bbox[1] + d.bbox[3] / 2); const k = `${z}${d.key}`; cur.set(k, (cur.get(k) ?? 0) + 1); }
      for (const s of setores) for (const cl of classes) {
        const k = `${s.label}${cl}`; const n = cur.get(k) ?? 0;
        const a = this.accum.get(k) ?? { setor: s.label, classe: cl, samples: 0, countSum: 0, peak: 0, present: 0 };
        a.samples++; a.countSum += n; a.peak = Math.max(a.peak, n); if (n > 0) a.present++;
        this.accum.set(k, a);

        if (cl === "pessoa") continue; // presença de pessoa é ruidosa
        const present = n > 0;
        const st = this.presence.get(k);
        if (!st) { this.presence.set(k, { confirmed: present, pending: present, since: now }); continue; }
        if (present !== st.pending) { st.pending = present; st.since = now; }
        else if (present !== st.confirmed && now - st.since >= PRESENCE_CONFIRM_MS) {
          st.confirmed = present;
          const nome = objClass(cl)?.label ?? cl, emoji = objClass(cl)?.emoji ?? "";
          events.push({ type: present ? "entrada" : "saida", setor: s.label, classe: cl, ts: Date.now() });
    ...
    const detectMs = this.pendingDetectMs; this.pendingDetectMs = null;
    return { dets: this.dets, counts, matrix, carregando, backend: objectBackend(), detectMs, samples, events, alerts };
  }

  dispose(): void { /* worker compartilhado; nada a liberar por instância */ }
}
