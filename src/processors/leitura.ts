...
export class LeituraProcessor implements Disposable {
  ...
  constructor() { void ensureDecoder().then(() => { this.kind = decoderKind(); }); }

  process(zone: LeituraZone, ctx: LeituraCtx): LeituraResult {
    const f = ctx.frame, now = ctx.now;
    const sx = Math.round(zone.x * f.w), sy = Math.round(zone.y * f.h);
    const sw = Math.max(1, Math.round(zone.w * f.w)), sh = Math.max(1, Math.round(zone.h * f.h));

    // ── PASSAGEM de caixa (motion na ROI; borda de subida = entrou caixa) ──
    const mpw = R.motionProcWidth, mph = Math.max(1, Math.round((mpw * sh) / sw));
    if (this.motion.width !== mpw || this.motion.height !== mph) { this.motion.width = mpw; this.motion.height = mph; this.prevLuma = null; this.curLuma = null; }
    ...
    if (mctx) {
      ...
      for (let i = 0, j = 0; i < mdata.length; i += 4, j++) luma[j] = 0.299 * mdata[i] + 0.587 * mdata[i + 1] + 0.114 * mdata[i + 2];
      const prev = this.prevLuma;
      if (prev) {
        let changed = 0; const total = luma.length;
        for (let k = 0; k < total; k++) if (Math.abs(luma[k] - prev[k]) > R.motionPixelDelta) changed++;
        const ratio = total > 0 ? changed / total : 0;
        if (this.passState === "clear" && ratio > R.passEnterRatio) {
          this.passState = "occupied";
          if (now - this.lastPassAt > R.passDebounceMs) { this.lastPassAt = now; this.passesCount++; this.pendingPasses.push({ cameraId: ctx.cameraId, ponto: zone.ponto, ts: Date.now() }); }
    ...
    if (!this.decoding && now - this.lastDecodeAt > R.decodeIntervalMs) {
      this.lastDecodeAt = now; this.decoding = true;
      const roiW = Math.min(sw, 1080), roiH = Math.max(1, Math.round((roiW * sh) / sw));
      if (this.proc.width !== roiW || this.proc.height !== roiH) { this.proc.width = roiW; this.proc.height = roiH; }
      const pctx = this.proc.getContext("2d", { willReadFrequently: true });
      if (pctx) {
        pctx.drawImage(f.el, sx, sy, sw, sh, 0, 0, roiW, roiH);
        const t0 = performance.now();
        void decodeFromCanvas(this.proc).then((res) => {
          this.pendingDecodeMs = performance.now() - t0;
          if (!res) return;
          const ts = Date.now();
          const last = this.lastEmit;
          if (last && last.code === res.code && ts - last.ts < R.dedupWindowMs) { last.ts = ts; return; } // mesma caixa em cena
          this.lastEmit = { code: res.code, ts };
          this.readTs.push(ts);
          this.boxesCount++;
          this.pendingReads.push({ cameraId: ctx.cameraId, cameraLabel: ctx.cameraLabel, ponto: zone.ponto, code: res.code, format: res.format, ts });
        }).catch(() => {}).finally(() => { this.decoding = false; });
    ...
    const reads = this.pendingReads; this.pendingReads = [];
    const passes = this.pendingPasses; this.pendingPasses = [];
    const decodeMs = this.pendingDecodeMs; this.pendingDecodeMs = null;
    return {
      reads, passes, perMin,
      ...
      kind: this.kind, decodeMs,
    };
  }

  dispose(): void { /* o worker de decode é compartilhado; nada a liberar por instância */ }
}
