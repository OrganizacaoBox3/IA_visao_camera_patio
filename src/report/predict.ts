...
export type AlertPrediction =
  | { status: "ok"; perDay: number; baselinePerDay: number; days: number }
  | { status: "no-data" };
...
export function predictAlertsPerDay(ds: Dataset, area: string, sensitivity: number): AlertPrediction {
  ...
  return { status: "ok", perDay: round1(Math.max(0, baselinePerDay * scale)), baselinePerDay: round1(baselinePerDay), days: ds.days };
}
