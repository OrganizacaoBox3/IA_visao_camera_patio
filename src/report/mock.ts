...
export type Cell = { area: string; dayIndex: number; hour: number; idleMin: number; alerts: number; activePct: number; atividade?: string };
export type Dataset = { days: number; areas: string[]; cameraOf: Record<string, string>; cells: Cell[]; startMs: number };
...
function inShift(hour: number, shift: Shift | "Todos") { return shift === "Todos" || shiftOf(hour) === shift; }

/** Recorta janelas current/previous conforme filtros. */
export function windows(ds: Dataset, f: Filters) {
  const W = periodDays[f.period];
  const curLo = ds.days - W, curHi = ds.days - 1;
  const prevLo = ds.days - 2 * W, prevHi = ds.days - W - 1;
  const sel = (lo: number, hi: number) => ds.cells.filter((c) =>
    c.dayIndex >= lo && c.dayIndex <= hi && inShift(c.hour, f.shift) && (f.area === "Todas" || c.area === f.area));
  return { current: sel(curLo, curHi), previous: sel(Math.max(0, prevLo), Math.max(-1, prevHi)), W };
}

export type Kpis = { idleMin: number; alerts: number; topArea: string; peakHour: number; activePct: number };
...
export function kpis(cells: Cell[]): Kpis {
  ...
  let pctSum = 0;
  for (const c of cells) { byArea.set(c.area, (byArea.get(c.area) ?? 0) + c.idleMin); byHour[c.hour] += c.idleMin; pctSum += c.activePct; }
...
export function ranking(cells: Cell[], areas: string[]) {
  const rows = areas.map((area) => {
    let idleMin = 0, alerts = 0;
    for (const c of cells) if (c.area === area) { idleMin += c.idleMin; alerts += c.alerts; }
    return { area, idleMin, alerts };
  }).filter((r) => r.idleMin > 0).sort((a, b) => b.idleMin - a.idleMin);
...
export function evolution(ds: Dataset, f: Filters, lastN = 14) {
  ...
  for (let d = lo; d < ds.days; d++) {
    let idleMin = 0;
    for (const c of ds.cells) if (c.dayIndex === d && inShift(c.hour, f.shift) && (f.area === "Todas" || c.area === f.area)) idleMin += c.idleMin;
    const date = new Date(ds.startMs + d * 86_400_000);
    out.push({ dayIndex: d, label: date.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }), idleMin });
  }
  const max = Math.max(1, ...out.map((o) => o.idleMin));
  return { bars: out, max };
}

export type EventRow = { ts: number; area: string; camera: string; durationMin: number; shift: Shift };
...
export function insights(cells: Cell[], k: Kpis): string[] {
  ...
  const out: string[] = [];
  if (top) out.push(`${top[0]} concentra ${Math.round((top[1] / total) * 100)}% do tempo parado do período.`);
  out.push(`Pico de ociosidade às ${String(k.peakHour).padStart(2, "0")}h — concentrar atenção/efetivo nessa janela.`);
  if (k.alerts > 0) out.push(`${k.alerts} alertas no período; revisar limites por área para reduzir ruído.`);
  return out;
}
...
export type ReadingCell = { ponto: string; dayIndex: number; hour: number; boxes: number; reads: number; multiReads: number; passages: number; perCamera: Record<string, number> };
export type ReadingDataset = { days: number; pontos: string[]; cameraLabels: Record<string, string>; cells: ReadingCell[]; startMs: number };
export type ReadingEventRow = { ts: number; ponto: string; code: string; cameras: number; shift: Shift };
export type ReadingFilters = { period: Period; shift: Shift | "Todos"; ponto: string | "Todos" };
export type ReadingKpis = { boxes: number; reads: number; multiReads: number; multiPct: number; passages: number; noReads: number; ratePct: number; topPonto: string; peakHour: number; pontos: number };
...
export function readingWindows(ds: ReadingDataset, f: ReadingFilters) {
  ...
  const prevLo = ds.days - 2 * W, prevHi = ds.days - W - 1;
  const sel = (lo: number, hi: number) => ds.cells.filter((c) =>
    c.dayIndex >= lo && c.dayIndex <= hi && inShift(c.hour, f.shift) && (f.ponto === "Todos" || c.ponto === f.ponto));
  return { current: sel(curLo, curHi), previous: sel(Math.max(0, prevLo), Math.max(-1, prevHi)), W };
}
...
export function readingKpis(cells: ReadingCell[]): ReadingKpis {
  ...
  const byHour = new Array(24).fill(0) as number[];
  for (const c of cells) { byPonto.set(c.ponto, (byPonto.get(c.ponto) ?? 0) + c.boxes); byHour[c.hour] += c.boxes; }
  const topPonto = [...byPonto.entries()].sort((a, b) => b[1] - a[1])[0]?.[0] ?? "—";
  const peakHour = byHour.some((v) => v > 0) ? byHour.indexOf(Math.max(...byHour)) : 0;
  return {
    boxes, reads, multiReads, multiPct: boxes ? Math.round((multiReads / boxes) * 100) : 0,
    passages, noReads, ratePct: passages ? Math.min(100, Math.round((boxes / passages) * 100)) : 100,
    topPonto, peakHour, pontos: byPonto.size,
  };
}
...
export function readingRanking(cells: ReadingCell[], pontos: string[]) {
  const rows = pontos.map((ponto) => {
    let boxes = 0, multiReads = 0, passages = 0;
    for (const c of cells) if (c.ponto === ponto) { boxes += c.boxes; multiReads += c.multiReads; passages += (c.passages ?? c.boxes); }
    const noReads = Math.max(0, passages - boxes);
    return { ponto, boxes, multiReads, noReads, ratePct: passages ? Math.min(100, Math.round((boxes / passages) * 100)) : 100 };
  }).filter((r) => r.boxes > 0 || r.noReads > 0).sort((a, b) => b.boxes - a.boxes);
...
export function readingByCamera(cells: ReadingCell[], labels: Record<string, string>) {
  ...
  for (const c of cells) for (const [cid, n] of Object.entries(c.perCamera)) m.set(cid, (m.get(cid) ?? 0) + n);
  const rows = [...m.entries()].map(([cid, reads]) => ({ camera: labels[cid] ?? cid, reads })).filter((r) => r.reads > 0).sort((a, b) => b.reads - a.reads);
...
export function readingEvolution(ds: ReadingDataset, f: ReadingFilters, lastN = 14) {
  ...
  for (let d = lo; d < ds.days; d++) {
    let boxes = 0;
    for (const c of ds.cells) if (c.dayIndex === d && inShift(c.hour, f.shift) && (f.ponto === "Todos" || c.ponto === f.ponto)) boxes += c.boxes;
    const date = new Date(ds.startMs + d * 86_400_000);
    out.push({ dayIndex: d, label: date.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }), boxes });
  }
...
export function readingInsights(k: ReadingKpis): string[] {
  const out: string[] = [];
  out.push(k.ratePct >= 98 ? `Taxa de leitura de ${k.ratePct}% — excelente cobertura.` : `Taxa de leitura de ${k.ratePct}%${k.noReads > 0 ? ` (${k.noReads.toLocaleString("pt-BR")} no-reads)` : ""}.`);
  if (k.topPonto !== "—") out.push(`${k.topPonto} é o ponto de maior volume.`);
  out.push(k.multiPct > 0 ? `${k.multiPct}% das caixas confirmadas por +1 câmera (redundância saudável).` : `Sem multi-leitura — avaliar cobertura de ângulos.`);
  return out;
}
...
export type ObjectCell = { setor: string; classe: string; dayIndex: number; hour: number; samples: number; countSum: number; peak: number; present: number };
export type ObjectDataset = { days: number; setores: string[]; classes: string[]; cells: ObjectCell[]; startMs: number };
export type ObjectEventRow = { ts: number; type: string; setor: string; classe: string; shift: Shift };
export type ObjectFilters = { period: Period; shift: Shift | "Todos"; setor: string | "Todos" };
export type ObjectKpis = { avgCount: number; peak: number; topClasse: string; presenceTopPct: number; setores: number; classes: number };

export function objectWindows(ds: ObjectDataset, f: ObjectFilters) {
  const W = periodDays[f.period];
  const sel = (lo: number, hi: number) => ds.cells.filter((c) =>
    c.dayIndex >= lo && c.dayIndex <= hi && inShift(c.hour, f.shift) && (f.setor === "Todos" || c.setor === f.setor));
  return { current: sel(ds.days - W, ds.days - 1), previous: sel(Math.max(0, ds.days - 2 * W), Math.max(-1, ds.days - W - 1)), W };
}
...
export function objectKpis(cells: ObjectCell[]): ObjectKpis {
  ...
  for (const c of cells) { const e = byClasse.get(c.classe) ?? { count: 0, present: 0, samples: 0 }; e.count += c.countSum; e.present += c.present; e.samples += c.samples; byClasse.set(c.classe, e); }
  const top = [...byClasse.entries()].sort((a, b) => b[1].count - a[1].count)[0];
  const topClasse = top?.[0] ?? "—";
  const presenceTopPct = top && top[1].samples ? Math.round((top[1].present / top[1].samples) * 100) : 0;
  return { avgCount: samples ? +(countSum / samples).toFixed(1) : 0, peak, topClasse, presenceTopPct, setores: new Set(cells.map((c) => c.setor)).size, classes: byClasse.size };
}
...
export function objectHeatmap(cells: ObjectCell[], classes: string[]) {
  const rows = classes.map((classe) => {
    const cnt = new Array(24).fill(0) as number[]; const smp = new Array(24).fill(0) as number[];
    for (const c of cells) if (c.classe === classe) { cnt[c.hour] += c.countSum; smp[c.hour] += c.samples; }
...
export function objectPresence(cells: ObjectCell[], setores: string[], classes: string[]) {
  ...
  for (const s of setores) { m[s] = {}; for (const cl of classes) {
    let present = 0, samples = 0;
    for (const c of cells) if (c.setor === s && c.classe === cl) { present += c.present; samples += c.samples; }
...
export function objectRanking(cells: ObjectCell[], setores: string[]) {
  const rows = setores.map((setor) => {
    let count = 0, samples = 0, peak = 0;
    for (const c of cells) if (c.setor === setor) { count += c.countSum; samples += c.samples; peak = Math.max(peak, c.peak); }
    return { setor, avg: samples ? +(count / samples).toFixed(1) : 0, peak };
  }).filter((r) => r.avg > 0 || r.peak > 0).sort((a, b) => b.avg - a.avg);
...
export function objectByClass(cells: ObjectCell[], classes: string[]) {
  const rows = classes.map((classe) => {
    ...
    for (const c of cells) if (c.classe === classe) { count += c.countSum; samples += c.samples; }
...
export function objectEvolution(ds: ObjectDataset, f: ObjectFilters, lastN = 14) {
  ...
  for (let d = lo; d < ds.days; d++) {
    let count = 0, samples = 0;
    for (const c of ds.cells) if (c.dayIndex === d && inShift(c.hour, f.shift) && (f.setor === "Todos" || c.setor === f.setor)) { count += c.countSum; samples += c.samples; }
    const date = new Date(ds.startMs + d * 86_400_000);
    out.push({ dayIndex: d, label: date.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }), avg: samples ? +(count / samples).toFixed(1) : 0 });
  }
...
export function objectInsights(k: ObjectKpis, loads: number): string[] {
  const out: string[] = [];
  if (k.topClasse !== "—") out.push(`${k.topClasse} é o objeto mais presente (${k.presenceTopPct}% do tempo).`);
...
export type FadigaCell = { posto: string; dayIndex: number; hour: number; samples: number; ok: number; fadiga: number; celular: number; duplo: number; earSum: number; earSamples: number };
export type FadigaDataset = { days: number; postos: string[]; cells: FadigaCell[]; startMs: number };
...
export type FadigaKpis = { alertPct: number; okPct: number; avgEar: number; peakHour: number; alertMin: number; postos: number };

export function fadigaWindows(ds: FadigaDataset, f: FadigaFilters) {
  const W = periodDays[f.period];
  const sel = (lo: number, hi: number) => ds.cells.filter((c) =>
    c.dayIndex >= lo && c.dayIndex <= hi && inShift(c.hour, f.shift) && (f.posto === "Todos" || c.posto === f.posto));
  return { current: sel(ds.days - W, ds.days - 1), previous: sel(Math.max(0, ds.days - 2 * W), Math.max(-1, ds.days - W - 1)), W };
}
...
export function fadigaKpis(cells: FadigaCell[]): FadigaKpis {
  ...
  const earSum = cells.reduce((a, c) => a + c.earSum, 0), earSamples = cells.reduce((a, c) => a + c.earSamples, 0);
  const byHour = new Array(24).fill(0) as number[];
  ...
  return {
    ...
    peakHour, alertMin: Math.round(alert / 60), postos: new Set(cells.map((c) => c.posto)).size,
  };
}
...
export function fadigaEvolution(ds: FadigaDataset, f: FadigaFilters, lastN = 14) {
  ...
  for (let d = lo; d < ds.days; d++) {
    let samples = 0, alert = 0;
    for (const c of ds.cells) if (c.dayIndex === d && inShift(c.hour, f.shift) && (f.posto === "Todos" || c.posto === f.posto)) { samples += c.samples; alert += c.fadiga + c.celular + c.duplo; }
    const date = new Date(ds.startMs + d * 86_400_000);
    out.push({ dayIndex: d, label: date.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }), pct: samples ? Math.round((alert / samples) * 100) : 0 });
  }
...
export function fadigaInsights(k: FadigaKpis, occFadiga: number, occCelular: number): string[] {
  const out: string[] = [];
  out.push(k.alertPct <= 2 ? `Operação saudável: ${k.okPct}% do tempo sem alerta.` : `${k.alertPct}% do tempo em alerta (${k.alertMin} min).`);
  out.push(`Pico de risco às ${String(k.peakHour).padStart(2, "0")}h.`);
  if (occFadiga + occCelular > 0) out.push(`${occFadiga} ocorrência(s) de fadiga, ${occCelular} de celular.`);
  return out;
}
...
export type AlarmFilters = { period: Period; priority: AlarmPriority | "Todas"; state: AlarmState | "Todos" };
...
export type AlarmKpis = { total: number; critical: number; high: number; advisory: number; news: number };

export function alarmKpis(events: AlarmEvent[]): AlarmKpis {
  let critical = 0, high = 0, advisory = 0, news = 0;
...
export function alarmDayStart(ts: number): number { return Math.floor(ts / ALARM_DAY) * ALARM_DAY; }
...
export function alarmTrend(events: AlarmEvent[], lastN = 14, now: number = Date.now()) {
  ...
  for (let i = lastN - 1; i >= 0; i--) {
    ...
    bars.push({ dayStart, label: new Date(dayStart).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" }), count: 0, critical: 0 });
  }
  const idx = new Map(bars.map((b, i) => [b.dayStart, i] as const));
  for (const e of events) {
    const i = idx.get(alarmDayStart(e.ts));
    if (i != null) { bars[i].count++; if (e.priority === "critical") bars[i].critical++; }
  }
