...
import { FadigaProcessor, type FadigaModelState, type FadigaCounters, type FadigaFlags, type FadigaThresholds, FADIGA_FLAGS_ALL } from "./processors/fadiga";
...
const RISK_CLS: Record<RiskState, string> = { OK: "ATIVA", ALERTA_FADIGA: "OCIOSA", ALERTA_CELULAR: "OCIOSA", ALERTA_DUPLO: "ALERTA" };
const DETECTORS = [["face", "Face"], ["hands", "Mãos"], ["phone", "Celular"], ["risk", "Risco"]] as const;
...
export function FadigaView({ label, getFrame, mode, onOpen, onClose, onAlert, onSample, onEvent }: Props) {
  ...
  const [counters, setCounters] = useState<FadigaCounters>({ fadiga: 0, bocejo: 0, celular: 0, duplo: 0 });
  ...
  useEffect(() => { engineRef.current.setThresholds(thresholds); saveFadigaThresholds(thresholds); }, [thresholds]);
  useEffect(() => { onAlertRef.current = onAlert; }, [onAlert]);
  useEffect(() => { onSampleRef.current = onSample; }, [onSample]);
  useEffect(() => { onEventRef.current = onEvent; }, [onEvent]);
  useEffect(() => { mutedRef.current = muted; }, [muted]);
  useEffect(() => { flagsRef.current = flags; }, [flags]);
  useEffect(() => { const e = engineRef.current; return () => e.dispose(); }, []);
  ...
  function beep() {
    ...
    try {
      ...
      const a = audioRef.current; if (a.state === "suspended") void a.resume();
      const osc = a.createOscillator(), gain = a.createGain();
      osc.frequency.value = APP_CONFIG.audio.alertFrequencyHz; osc.connect(gain); gain.connect(a.destination);
      gain.gain.setValueAtTime(0.0001, a.currentTime);
      gain.gain.exponentialRampToValueAtTime(0.18, a.currentTime + 0.015);
      gain.gain.exponentialRampToValueAtTime(0.0001, a.currentTime + APP_CONFIG.audio.alertDurationMs / 1000);
      osc.start(); osc.stop(a.currentTime + APP_CONFIG.audio.alertDurationMs / 1000);
    } catch { /* política de áudio */ }
  }
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      const s = r.snapshot;
      if (r.alertRisk) { ackRef.current = false; setAck(false); onAlertRef.current?.(`⚠ ${label}: ${RISK_LABEL[r.alertRisk]}`); }
      if (s.risk === "OK") { if (ackRef.current) { ackRef.current = false; setAck(false); } }
      else {
        if (s.signal === "JOINHA" && !ackRef.current) { ackRef.current = true; setAck(true); onAlertRef.current?.(`✋ ${label}: alerta reconhecido (gesto 👍)`); }
      ...
      if (now - lastUiRef.current > uiEvery) {
        ...
        const s = r.snapshot;
        setEar(s.ear); setMar(s.mar); setYawn(s.yawn); setSignal(s.signal); setHandCount(s.handCount);
        setPhone(s.phone); setRisk(s.risk); setFaceState(s.faceState); setCounters(s.counters);
        const mt = meterRef.current; setFps(Math.round(mt.fps)); setFaceLat(Math.round(mt.avgProcMs));
        setHandLat(Math.round(handMeterRef.current.avgProcMs)); setObjLat(Math.round(objMeterRef.current.avgProcMs));
      }
    };
    rafRef.current = requestAnimationFrame(loop);
  ...
  const status = faceState === "error" ? { txt: "modelo falhou", cls: "ALERTA" } : loading ? { txt: "carregando", cls: "VAZIA" } : { txt: RISK_LABEL[risk], cls: RISK_CLS[risk] };
  ...
  return (
    <div className="cam fadiga-cam">
      <header className="cam-head">
        <div className="cam-title"><b>{label}</b><span className="muted">operador · fadiga</span></div>
        ...
        <aside className="cam-drawer">
          <div className="read-now">
            ...
            <div className="read-now-meta">
              <span>EAR <b>{ear == null ? "--" : ear.toFixed(3)}</b></span>
              <span>MAR <b>{mar == null ? "--" : mar.toFixed(3)}</b>{yawn ? " (bocejo)" : ""}</span>
            </div>
          </div>
          <div className="read-flow-h">Sinais</div>
          <div className="fadiga-signals">
            <div className="fs-row"><span>Celular</span><span className={phone ? "on" : ""}>{phone ? `sim ${Math.round(phone.score * 100)}%` : "não"}</span></div>
            <div className="fs-row"><span>Gesto de mão</span><span>{signal} ({handCount})</span></div>
            <div className="fs-row"><span>Modelo facial</span><span>{faceState === "ready" ? "ativo" : faceState === "loading" ? "carregando…" : "falhou"}</span></div>
          </div>
          <div className="read-flow-h">Ocorrências</div>
          <div className="fadiga-signals">
            <div className="fs-row"><span>Fadiga</span><span>{counters.fadiga}</span></div>
            <div className="fs-row"><span>Bocejo</span><span>{counters.bocejo}</span></div>
            <div className="fs-row"><span>Celular</span><span>{counters.celular}</span></div>
            <div className="fs-row"><span>Duplo</span><span>{counters.duplo}</span></div>
          </div>
      ...
      <div className="cam-kpibar">
        <span className="kb">⦿ risco <b>{RISK_LABEL[risk]}</b></span>
        <span className="kb">EAR <b>{ear == null ? "--" : ear.toFixed(2)}</b></span>
        <span className="kb">MAR <b>{mar == null ? "--" : mar.toFixed(2)}</b></span>
        <span className="kb">📱 <b>{phone ? "sim" : "não"}</b></span>
        <span className="kb">{muted ? "🔇" : "🔊"}{ack ? " ✋" : ""}</span>
        <span className="kb muted">FPS {fps} · face {faceLat}ms · mãos {handLat}ms · cel {objLat}ms</span>
      </div>
    </div>
  );
}
