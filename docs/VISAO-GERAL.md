...
> Origem do conceito: `document_pdf.pdf` (proposta Box3.work / Tiago Lucena). Planos/análises em `docs/`, manuais em `docs/manuais/`.
...
- **Câmera IP/RTSP:** ingestão no hub via ffmpeg → frames JPEG (vira câmera comum). Manuais em `docs/manuais/`.
...
```
Câmera IP: instalar ffmpeg + `server/rtsp.sources.json` (ver `docs/manuais/`).
