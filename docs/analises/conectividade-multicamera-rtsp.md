...
> documentação `docs/arquitetura/02` e `docs/arquitetura/06`.
...
- Sem banco de câmeras: nenhuma tabela/persistência de fontes (diferente de
  ...
  docs/arquitetura/02 §5-6) — **não compartilhado** entre operadores nem servidor.
...
> O artefato `docs/analises/cameras-fontes-publicas-demo.md` **ainda não existe**
> (a pasta `docs/analises/` estava vazia na elaboração desta avaliação). Quando for
...
2. **Criar `server/rtsp.sources.json`** copiando o modelo
   ...
   credenciais; ver também docs/arquitetura/06 §10.2.) Formato:
   ```json
   [
...
|---|------|---------|-------|-------------|
| 1 | Curar e validar **URLs públicas/demo** + criar `docs/analises/cameras-fontes-publicas-demo.md` | P | Baixo | Muitos streams públicos caem/mudam; testar item 4 acima |
...
- **Segurança/LGPD**: URLs RTSP com credenciais e `rtsp.sources.json` são
  ...
  controlar acesso (docs/arquitetura/06 §10).
