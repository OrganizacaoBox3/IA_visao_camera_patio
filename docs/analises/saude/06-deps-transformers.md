...
Não mudar nada agora; **registrar risco-aceito** (ADR em `docs/analises/decisoes/`) apoiado na §2: browser + worker sandbox + modelo de fonte fixa HTTPS + CSP restritivo + sem PII/servidor. Mitigações já presentes (CSP `connect-src` à HF; `allowLocalModels=false`; fallback coco-ssd no `detector.ts`).
