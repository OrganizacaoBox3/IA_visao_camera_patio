...
O `git log` mostra **30 commits** organizados explicitamente em **ondas de feature A–G** (changelog em `docs/analises/implementacao-changelog.md`), todas validadas pelo trio **`tsc` + `vite build` + Playwright e2e** — nunca por lint/CI/unit. **(E)** `git log --oneline`:

- **Import inicial** (`529512b`) → **docs/produto/benchmark** → **Onda A** (política de alarme, going-gray, previsão) → **Onda B** (RBAC, fila de alarmes, eventos) → **Onda C** (presets, tripwires, ocupação, shelving, métricas) → **persistência compartilhada** (views/tripwires, ADRs) → **migração Radix** (fundação → telas → responsividade) → **fix Radix Select-em-Dialog** → **`CLAUDE.md`**.
