...
- **Relatório:** modos do SegmentedControl (role=**radio**, não button) — **"Resumo"**, **"Atividade"**, **"Linhas"**, **"Alarmes"** (as dimensões só aparecem com dado no histórico) · `tablist` com `aria-label` **"Seção"** e **4 tabs**: **"Quando para"**, **"Onde para"**, **"Tendência"**, **"Eventos (n)"** · conteúdos de painel: "horários críticos", "Por área", "Tendência (14 dias)". No modo **Linhas**: Select **"Linha de contagem"** · texto **/Saldo \(entradas − saídas\)/** com **/não meta/** ao lado. *(2026-09-10: "Fluxo de pessoas" era a 5ª aba de Atividade e virou MODO — o filtro daquele modo é ÁREA, que não se aplica a cruzamento de linha.)*
