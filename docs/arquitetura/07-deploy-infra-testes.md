...
Fluxo em produção (resumo de `docs/produto/deploy-digitalocean.md`):
...
Decisões-chave (de `docs/produto/deploy-digitalocean.md:28`):
...
> documentado (`docs/produto/deploy-digitalocean.md`) e os arquivos em `deploy/` usam **nginx**. O Caddy
...
> **Discrepância (a confirmar):** `docs/produto/deploy-digitalocean.md:157` menciona `PANEL_PASSWORD`
...
Runbook: `docs/produto/deploy-atualizacao-2026-07.md §11`.
...
3b. **Proxy `/go2rtc/` → `127.0.0.1:1984`** com upgrade WebSocket (Onda 2) — vídeo WebRTC same-origin
   ...
   pronto em `docs/produto/deploy-atualizacao-2026-07.md §11.2` / `docs/produto/deploy-digitalocean.md §5.1`. Same-origin
   → o CSP não muda.
...
### 4.3 DigitalOcean — resumo do roteiro (`docs/produto/deploy-digitalocean.md`)
...
> Baseado em `docs/produto/deploy-digitalocean.md` + arquivos em `deploy/`.
...
Guias práticos existentes em `docs/produto/manuais/` (apenas leitura; índice em
`docs/produto/manuais/README.md`). Resumo:

### 9.1 Câmeras IP / RTSP (geral) — `docs/produto/manuais/manual-camera-rtsp.md`
...
### 9.2 Intelbras (RTSP) — `docs/produto/manuais/manual-intelbras-rtsp.md`
...
### 9.3 Leitores industriais (Sick / Cognex) — `docs/produto/manuais/manual-leitores-industriais.md`
...
| `docs/produto/deploy-digitalocean.md` | Roteiro de deploy na VPS (fonte primária). |
| `docs/produto/manuais/*` | Manuais de câmeras RTSP / Intelbras / leitores industriais. |
