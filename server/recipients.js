...
function saveFile() { try { fs.writeFileSync(FILE, JSON.stringify(list, null, 2)); } catch (e) { console.error("[recipients] falha ao salvar:", e.message); } }
async function persist(r) {
  ...
  await db.query(
    ...
    [r.id, r.nome, r.numero, r.ativo, r.somenteCriticos, JSON.stringify(r.tipos || []), r.criadoEm ?? Date.now()]
  );
}
async function persistDelete(id) { if (!usingPg) return saveFile(); await db.query("delete from recipients where id=$1", [id]); }

async function init() {
  if (db.configured()) {
    try {
      const r = await db.query(`select id, nome, numero, ativo, somente_criticos as "somenteCriticos", tipos, criado_em as "criadoEm" from recipients order by criado_em asc nulls first`);
      list = r.rows; usingPg = true;
      console.log(`[recipients] ${list.length} destinatário(s) do Postgres`);
      return;
    } catch (e) { console.error("[recipients] Postgres indisponível, usando JSON:", e.message); }
  }
  usingPg = false;
  try { const a = JSON.parse(fs.readFileSync(FILE, "utf8")); if (Array.isArray(a)) list = a; } catch { list = []; }
}
...
async function create({ nome, numero, somenteCriticos, tipos }) {
  ...
  const r = { id: "r" + crypto.randomBytes(5).toString("hex"), nome: String(nome || "").trim() || n, numero: n, ativo: true, somenteCriticos: somenteCriticos !== false, tipos: Array.isArray(tipos) ? tipos : [], criadoEm: Date.now() };
  list.push(r); await persist(r);
  return { recipient: r };
}
async function update(id, patch) {
  const r = list.find((x) => x.id === id); if (!r) return { error: "destinatário não encontrado" };
...
async function remove(id) { const n = list.length; list = list.filter((x) => x.id !== id); if (list.length !== n) await persistDelete(id); return { ok: true }; }
