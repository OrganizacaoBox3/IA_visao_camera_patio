  ...
  "ativ:samples": async (p) => {
    ...
    for (const sm of p.samples || []) {
      await db.query(
        ...
        [`${p.cameraId}|${sm.zoneId}|${hs}`, p.cameraId, sm.label, sm.atividade, hs, sm.idleMs, sm.frames, sm.activeFrames, sm.people]
      );
    }
  },
  "ativ:alert": async (a) => {
    await db.query(`insert into ativ_events (ts,camera_id,camera,area,atividade,duration_min,shift) values ($1,$2,$3,$4,$5,$6,$7)`,
      [a.ts, a.cameraId, a.cameraLabel, a.area, a.atividade, a.durationMin, a.shift]);
    const hs = hourOf(a.ts);
    await db.query(`insert into ativ_buckets (id,camera_id,area,atividade,hour_start,alerts) values ($1,$2,$3,$4,$5,1)
      on conflict (id) do update set alerts=ativ_buckets.alerts+1`,
      [`${a.cameraId}|${a.zoneId}|${hs}`, a.cameraId, a.area, a.atividade, hs]);
  },
  "read:read": async (r) => {
    const hs = hourOf(r.ts), id = `${r.ponto}|${hs}`;
    await db.query(
    ...
    if (r.newBox) await db.query(`insert into read_events (ts,ponto,code,cameras,shift) values ($1,$2,$3,1,$4)`, [r.ts, r.ponto, r.code, r.shift]);
  },
  "read:pass": async (p) => {
    const hs = hourOf(p.ts);
    await db.query(`insert into read_buckets (id,ponto,hour_start,passages) values ($1,$2,$3,1)
      on conflict (id) do update set passages=read_buckets.passages+1`, [`${p.ponto}|${hs}`, p.ponto, hs]);
  },
  "obj:samples": async (p) => {
    const hs = hourOf(Date.now());
    for (const sm of p.samples || []) {
      await db.query(
        ...
        [`${sm.setor}|${sm.classe}|${hs}`, sm.setor, sm.classe, hs, sm.samples, sm.countSum, sm.peak, sm.present]
      );
    }
  },
  "obj:event": async (e) => {
    await db.query(`insert into obj_events (ts,type,setor,classe,shift) values ($1,$2,$3,$4,$5)`, [e.ts, e.type, e.setor, e.classe, e.shift]);
  },
  "fad:samples": async (p) => {
    const hs = hourOf(Date.now());
    await db.query(
      ...
      [`${p.posto}|${hs}`, p.posto, hs, p.samples, p.ok, p.fadiga, p.celular, p.duplo, p.earSum, p.earSamples]
    );
  },
  "fad:event": async (e) => {
    await db.query(`insert into fad_events (ts,posto,type,shift) values ($1,$2,$3,$4)`, [e.ts, e.posto, e.type, e.shift]);
  },
};
...
async function buckets(kind) { if (!db.configured()) return []; return (await db.query(BUCKET_SQL[kind])).rows; }
async function events(kind) { if (!db.configured()) return []; return (await db.query(EVENT_SQL[kind])).rows; }

async function clear() {
  if (!db.configured()) return;
  await db.query(`truncate ativ_buckets, ativ_events, read_buckets, read_events, obj_buckets, obj_events, fad_buckets, fad_events`);
}
