...
function readBody(req, limit = 10_000) {
  return new Promise((resolve) => {
    let b = ""; req.on("data", (c) => { b += c; if (b.length > limit) req.destroy(); }); req.on("end", () => resolve(b));
  });
}
...
function requireAuth(req, res) {
  ...
  if (!u) { json(res, 401, { error: "não autenticado" }); return null; }
  return u;
}
...
const httpServer = createServer(async (req, res) => {
  ...
  try {
    ...
    if (mb && req.method === "GET") {
      if (!requireAuth(req, res)) return;
      return json(res, 200, mb[2] === "buckets" ? await pgstore.buckets(mb[1]) : await pgstore.events(mb[1]));
    }
    ...
    if (req.url === "/api/me") {
      ...
      if (req.method === "PATCH" || req.method === "PUT") { const r = await users.updateProfile(me.id, JSON.parse((await readBody(req)) || "{}")); return r.error ? json(res, 400, r) : json(res, 200, r.user); }
    }
    ...
    if (req.url === "/api/recipients") {
      if (req.method === "GET") { if (!requireSuper(req, res)) return; return json(res, 200, recipients.all()); }
      if (req.method === "POST") { if (!requireSuper(req, res)) return; const r = await recipients.create(JSON.parse((await readBody(req)) || "{}")); return r.error ? json(res, 400, r) : json(res, 201, r.recipient); }
    }
    const mr = req.url && req.url.match(/^\/api\/recipients\/([\w-]+)$/);
    if (mr) {
      const id = mr[1];
      if (req.method === "PATCH") { if (!requireSuper(req, res)) return; const r = await recipients.update(id, JSON.parse((await readBody(req)) || "{}")); return r.error ? json(res, 400, r) : json(res, 200, r.recipient); }
      if (req.method === "DELETE") { if (!requireSuper(req, res)) return; await recipients.remove(id); return json(res, 200, { ok: true }); }
    }

    // Configuração de notificações (superadmin): GET atual, PUT salva, POST preview (sem salvar)
    if (req.url === "/api/notif-settings") {
      if (req.method === "GET") { if (!requireSuper(req, res)) return; return json(res, 200, settings.get()); }
      if (req.method === "PUT" || req.method === "PATCH") { if (!requireSuper(req, res)) return; return json(res, 200, await settings.update(JSON.parse((await readBody(req)) || "{}"))); }
    }
    if (req.url === "/api/notif-preview" && req.method === "POST") {
      ...
      const out = {};
      for (const [tipo, txt] of Object.entries(samples)) out[tipo] = dispatch.formatWhatsApp(txt, dispatch.classify(txt), now, s);
    ...
    if (req.url === "/api/wa-test" && req.method === "POST") {
      ...
      try { await whatsapp.sendText(numero, "✅ Teste — Visão de Pátio: notificações de WhatsApp funcionando."); return json(res, 200, { ok: true }); }
      catch (e) { return json(res, 400, { error: e.message }); }
    }

    // Gestão de usuários (somente superadmin)
    if (req.url === "/api/users") {
      if (req.method === "GET") { if (!requireSuper(req, res)) return; return json(res, 200, users.publicList()); }
      if (req.method === "POST") { if (!requireSuper(req, res)) return; const r = await users.createUser(JSON.parse((await readBody(req)) || "{}")); return r.error ? json(res, 400, r) : json(res, 201, r.user); }
    }
    const m = req.url && req.url.match(/^\/api\/users\/([\w-]+)$/);
    if (m) {
      const id = m[1];
      if (req.method === "PATCH") { if (!requireSuper(req, res)) return; const r = await users.updateUser(id, JSON.parse((await readBody(req)) || "{}")); return r.error ? json(res, 400, r) : json(res, 200, r.user); }
      if (req.method === "DELETE") { if (!requireSuper(req, res)) return; const r = await users.removeUser(id); return r.error ? json(res, 400, r) : json(res, 200, { ok: true }); }
    }
    ...
    if (req.url === "/api/cameras") {
      if (req.method === "GET") { if (!requireSuper(req, res)) return; return json(res, 200, cameraStore.all()); }
      if (req.method === "POST") {
        if (!requireSuper(req, res)) return;
    ...
    if (mcam) {
      ...
      if (req.method === "PATCH") {
        ...
        if (r.error) return json(res, 400, r);
        if (r.camera.enabled === false) rtsp.removeSource(id); // desabilitada → para o stream
...
io.use((socket, next) => {
  ...
  if (role === "camera" && process.env.CAMERA_TOKEN && token === process.env.CAMERA_TOKEN) return next();
...
io.on("connection", (socket) => {
  ...
  if (role === "camera") {
    ...
    for (const c of cameraList()) if (c.kind !== "rtsp") socket.emit("camera-status", { id: c.id, state: "online", label: c.label, kind: "browser" });
...
(async () => {
  ...
  httpServer.listen(PORT, HOST, () => {
    ...
    console.log(alerts.andonEnabled() ? "[andon] webhook de alertas ATIVO" : "[andon] desligado (defina ALERT_WEBHOOK_URL para ligar)");
    console.log(whatsapp.enabled() ? "[whatsapp] habilitado — pareie pelo QR no painel" : "[whatsapp] desligado (defina WHATSAPP_ENABLED=1 para ligar)");
    whatsapp.init();
