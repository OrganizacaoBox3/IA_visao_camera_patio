...
function signToken(u) {
  const body = Buffer.from(
    JSON.stringify({ id: u.id, papel: u.papel, exp: Date.now() + TOKEN_TTL_MS }),
  ).toString("base64url");
...
function authenticate(usuario, senha) {
  const u = users.find(
    (x) => x.usuario.toLowerCase() === String(usuario || "").toLowerCase() && x.ativo,
  );
  ...
  if (db.configured()) {
    try {
      ...
      if (!users.length) {
        ...
        await persist(su);
        console.log(
          `[users] superadmin '${su.usuario}' criado no Postgres (bootstrap) — TROQUE a senha.`,
        );
      }
      console.log(`[users] ${users.length} usuário(s) do Postgres`);
      return;
    } catch (e) {
      console.error("[users] Postgres indisponível, usando JSON:", e.message);
    }
  }
  usingPg = false;
  try {
    const a = JSON.parse(fs.readFileSync(FILE, "utf8"));
    if (Array.isArray(a)) users = a;
  } catch {
    users = [];
  }
  if (!users.length) {
...
function normalizeRole(papel) {
  return ROLES.includes(papel) ? papel : "usuario";
}
...
async function updateUser(id, patch) {
  ...
  const otherSupers = users.filter(
    (x) => x.id !== id && x.papel === "superadmin" && x.ativo,
  ).length;
...
async function updateProfile(id, patch) {
  ...
  if (patch.filtros && typeof patch.filtros === "object")
    u.filtros = {
      ativo: !!patch.filtros.ativo,
      somenteCriticos: !!patch.filtros.somenteCriticos,
