// PORT de src/vision/counting.test.ts — os MESMOS cenários, contra o port CJS
// server/analysis/counting.js. Mudanças de comportamento devem ser feitas no TS
// de origem e re-portadas; estes testes garantem a paridade do port.
// Determinísticos: usam o parâmetro `now` (sem relógio real) e geometria fixa.
// Convenção (seta a→b; esquerda→direita = "in") — ver src/vision/counting.test-notes.md.
...
const vWire = { id: "w", a: { x: 0.5, y: 0 }, b: { x: 0.5, y: 1 } };
...
describe("createCounter — contagem direcional", () => {
  ...
  it("linha invertida (a↔b): o mesmo movimento troca in↔out", () => {
    const inv = { id: "w", a: vWire.b, b: vWire.a }; // seta p/ CIMA
...
describe("createCounter — âncora no PÉ do bbox (item 1.4)", () => {
  ...
  const hWire = { id: "h", a: { x: 0, y: 0.5 }, b: { x: 1, y: 0.5 } };
