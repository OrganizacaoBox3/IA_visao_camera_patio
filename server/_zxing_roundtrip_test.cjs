...
for (const [code, scale, quiet] of [
  ["7891234567895", 3, 11],
  ["7899876543215", 3, 10],
  ["4006381333931", 4, 12],
]) {
  try {
    const { lum, W, H } = raster1D(encodeEAN13(code), scale, quiet, 80);
    const o = decodeLikeApp(lum, W, H);
    check(`EAN_13 esc${scale} (${o.format})`, code, o.text);
  } catch (e) {
    total++;
    console.log(`✗ EAN_13 esc${scale} erro: ${(e && e.message) || e}`);
  }
}
